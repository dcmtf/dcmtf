...
from src.vae_abl import vaeabl
...
class dcmtfsinglepass(nn.Module): #VAE
    ...
    def __network_construction(self):
        assert self.abl_category in ["None","aec_remove","ortho_remove"]
        #Construct:
        #dict_vae
        #dict_ffnu_cat
        #dict_ffn_clust
            ...
            if self.is_load_init:
                ...
                for e_id in self.G.keys():
                    ...
                    for x_id in self.G[e_id]:
                        ...
                        if self.abl_category in ["aec_remove"]:
                            vae_e = vaeabl(cur_input_dim,cur_k_list,cur_actf_list,is_real)
                        else:
    ...
    def __print_input(self):
        ...
        print("###################################")
        print("abl_category:", self.abl_category)
        print("###################################")
        print("G: ")
        print(self.G)
        print("#")
    ...
    def __init__(self,\
        ...
        model_dir, abl_category=None, is_single_pass=True):
        super(dcmtfsinglepass, self).__init__()
        self.G = G
        self.X_data = X_data
        self.X_meta  = X_meta
        ...
        self.abl_category = abl_category
        assert is_single_pass == True
    ...
    def calc_kmeans_sil_score_c(self):
        print("#")
        print("calc_kmeans_sil_score - C: ")
        print("#")
        dict_sil = {}
        for e_id in self.G.keys():
            # if e_id == "g":
    ...
    def calc_kmeans_sil_score_u(self):
        print("#")
        print("calc_kmeans_sil_score - C: ")
        print("#")
        dict_sil = {}
        for e_id in self.G.keys():
            ...
            #     y_pred_e_sel = y_pred_e[np.array(self.y_val_dict[e_id])>= 0]
            #     #
            #     I_ortho_temp = self.dict_U[e_id].data.cpu().numpy()
            #     I_ortho_temp_sel = I_ortho_temp[np.array(self.y_val_dict[e_id]) >= 0]
            #     #
            #     #cur_sil = silhouette_score(I_ortho_temp_sel, y_pred_e_sel, metric='euclidean') 
            #     U_dist = cdist(I_ortho_temp_sel, I_ortho_temp_sel, metric = "euclidean")
    ...
    def fit(self):
        ...
        try:
            ...
            while True:
                ...
                for cur_batch in list_batch_ids:
                    ...
                    # single pass
                    #############
                    assert self.is_single_pass == True
                    # (1) Forward pass - VAEs
                    dict_C_enc = {}
                    dict_C_dec = {}
                    dict_mu = {}
                    ...
                    for e_id in self.G.keys():
                        ...
                        for x_id in self.G[e_id]:
                            ...
                            if not self.abl_category in ["aec_remove"]:
                                ...
                                dict_loss_aec_pass1[cur_id] = torch.tensor(0).float()
                                if epoch == 1:
                                    dict_loss_aec_metadata_pass1[cur_id]["start"] = torch.tensor(0).float() 
                    #
                    ...
                    for e_id in self.G.keys():
                        ...
                        if not self.abl_category in ["ortho_remove"]:
                            ...
                            dict_loss_regl[e_id] = torch.tensor(0).float() 
                            if epoch == 1:
                                dict_loss_regl_metadata[e_id]["start"] = torch.tensor(0).float()                     
                    #
                    #opt data
                    optimizer_data.step()
                    ...
                    #             dict_loss_aec_pass2[cur_id] = torch.tensor(0).float() 
                    #             if epoch == 1:
                    #                 dict_loss_aec_metadata_pass2[cur_id]["start"] = torch.tensor(0).float() 
                    # #
                    ...
                    loss_epoch_pass1 += loss_aec_pass1 + loss_recons + loss_regl
                ...
                for e_id in self.G.keys():
                    for x_id in self.G[e_id]:
                        ...
                        out_str_aec2+= str(round(0.0000,4))        
                out_str_recons = ""
                for x_id in self.X_meta.keys():
                    out_str_recons+= str(", ")
        ...
        self.fit_time = (fit_end_time-fit_start_time) 
        print("###")
        print("Fit Took ",self.fit_time," secs.")
        print("Fit Took ",round(self.fit_time/60.0,4)," mins.")
        print("###")