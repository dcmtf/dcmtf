...
elif dataset_id == "20ng":
   num_batches = 10
   data_version_name = "1"
   fname += "data_dict_20ng_p2.pkl.pbz2"
else:
...
if dataset_id in ["wiki1","wiki2","wiki3","wiki4"]:
    ...
    X_dw_norm = data_dict["matrices"]['X_dw_norm']
    X_dc_bin = data_dict["matrices"]['X_dc_bin']
    #
    print("#")
    print("Before pp:")
    print("#")
    print("X_dw_norm:")
    print("min: ",X_dw_norm.min()," max: ",X_dw_norm.max()," mean: ",X_dw_norm.mean()," sd: ",X_dw_norm.std())
    print("X_dc_bin:")
    print("min: ",X_dc_bin.min()," max: ",X_dc_bin.max()," mean: ",X_dc_bin.mean()," sd: ",X_dc_bin.std())
    ...
    scaler1 = StandardScaler()
    #scaler1 = Normalizer()
    X_dc = scaler1.fit_transform(X_dc_bin)
    #scaler2 = StandardScaler()
    #scaler2 = Normalizer()
    #X_dw = scaler2.fit_transform(X_dw_norm)
    X_dw = X_dw_norm
    ...
    print("#")
    print("X_dw:")
    print("min: ",X_dw.min()," max: ",X_dw.max()," mean: ",X_dw.mean()," sd: ",X_dw.std())
    print("X_dc:")
    print("min: ",X_dc.min()," max: ",X_dc.max()," mean: ",X_dc.mean()," sd: ",X_dc.std())

    y_d = data_dict["labels"]["d"]

    print("#")
    print("X_dw.shape: ",X_dw.shape)
    print("X_dc.shape: ",X_dc.shape)
    print("#")
    print("y_d.shape: ",len(y_d))
    print("#")

    ####### 2 entities #####

    # dict_e_size = {}
    # dict_e_size["d"] = X_dw.shape[0]
    # dict_e_size["w"] = X_dw.shape[1]
    
    # X_dw = torch.from_numpy(X_dw).float()

    # if is_gpu:
    #     X_dw = X_dw.cuda()

    # dict_num_clusters =  {"d":4,"w":8}
    ...
    # y_val_dict = {}
    # y_val_dict["d"] = np.array(y_d)
    # y_val_dict["w"] = np.array([])

    ####### 3 entities #####
    dict_e_size = {}
    dict_e_size["d"] = X_dw.shape[0]
    dict_e_size["w"] = X_dw.shape[1]
    dict_e_size["c"] = X_dc.shape[1]
...
if dataset_id in ["pubmed"]:
    ...
    for i in np.arange(num_runs):
        ...
        dcmtf_instance = dcmtf(G, X_data, X_data_bef_pp, X_data_size_fac, X_meta, X_dtype,\
                ...
                model_dir=out_dir,abl_category=abl_category)
        #
        dcmtf_instance.fit()
        #
